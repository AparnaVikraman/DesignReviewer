...
def usage_to_token_counts(usage: Any) -> tuple[int, int, int]:
    if usage is None:
        return 0, 0, 0
    return (
...
class LLMClient:
    ...
    def complete_stream(
        ...
        *,
        ...
        usage_input, usage_output, usage_total = 0, 0, 0

        logger.info(
            "LLM stream starting request_id=%s model=%s",
            request_id,
            resolved_model,
        )
        ...
        try:
            with self._client.responses.create(**kwargs) as stream:
                for event in stream:
                    if event.type == "response.output_text.delta":
                        ...
                        usage_input, usage_output, usage_total = usage_to_token_counts(
                            response.usage
                        )
        except Exception as exc:
            llm_error = _map_error(
            ...
            logger.error(
                "LLM stream failed request_id=%s error_type=%s message=%s",
                request_id,
                llm_error.error_type,
                llm_error.message,
            )
        ...
        latency_ms = round((time.perf_counter() - start_time) * 1000, 2)
        if usage_total == 0 and chunks:
            full_text = "".join(chunks)
            usage_output = count_tokens(full_text, response_model_name)
            usage_input = input_tokens_estimated
            usage_total = usage_input + usage_output

        logger.info(
            "LLM stream completed request_id=%s latency_ms=%.2f input_tokens=%d output_tokens=%d total_tokens=%d",
            request_id,
            latency_ms,
            usage_input,
            usage_output,
    ...
    def finalize_structured_response(
        ...
        *,
        ...
        fallback: Callable[[ValidationError], BaseModel] | None = None,
        max_repair_attempts: int = 1,
    ) -> tuple[BaseModel, dict[str, Any], Literal["direct", "repair", "fallback"]]:
        parsed = _parse_json_response(text, request_id=request_id)
        validated, parsed, _, validation_path = self._validate_with_repair_or_fallback(
            text=text,
            parsed=parsed,
            response_model=response_model,
            fallback=fallback,
        ...
        return validated, parsed, validation_path
